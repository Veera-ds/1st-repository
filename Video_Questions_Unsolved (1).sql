...
select count(cust_id) from market_star_schema.cust_dimen where state = 'west Bengal';
-- 6. Print the names of all customers who belong to West Bengal.
select customer_name from market_star_schema.cust_dimen where state = 'west Bengal';

-- -----------------------------------------------------------------------------------------------------------------
-- Operators

-- 1. Print the names of all customers who are either corporate or belong to Mumbai.
select customer_name, customer_segment,state from market_star_schema.cust_dimen where state = "Maharashtra" or Customer_Segment = "corporate" ;
-- 2. Print the names of all corporate customers from Mumbai.
select customer_name, state, customer_segment from market_star_schema.cust_dimen where state = "Maharashtra" and customer_segment = "corporate";
-- 3. List the details of all the customers from southern India: namely Tamil Nadu, Karnataka, Telangana and Kerala.
select * from market_star_schema.cust_dimen where state in ('Tamil Nadu','Karnataka','Telangana','Kerala');
-- 4. Print the details of all non-small-business customers.
select * from market_star_schema.cust_dimen where customer_segment = "small business";
-- 5. List the order ids of all those orders which caused losses.
select ord_id, profit from market_star_schema.market_fact_full where profit <0;
-- 6. List the orders with '_5' in their order ids and shipping costs between 10 and 15.
select customer_name from market_star_schema.cust_dimen where customer_name regexp "^[abcd].*er$" order by Customer_Name;-- and shipping_cost between 10 and 15; -- "%\[abcd]...er$%"--
...
select count(cust_id) as Total_no_customer, city from market_star_schema.cust_dimen group by city;
-- 3. Find the number of orders which have been sold at a loss.
select count(ord_id) as loss from market_star_schema.market_fact_full where profit <0;
-- 4. Find the total number of customers from Bihar in each segment.
select count(cust_id) as Total_customer, state from market_star_schema.cust_dimen where state ="bihar";
-- 5. Find the customers who incurred a shipping cost of more than 50.
select cust_id, customer_name, shipping_cost from market_star_schema.market_fact_full inner join market_star_schema.cust_dimen using (cust_id) where Shipping_Cost > 50;

-- -----------------------------------------------------------------------------------------------------------------
-- Ordering

-- 1. List the customer names in alphabetical order.
select customer_name from market_star_schema.cust_dimen order by Customer_Name desc;
-- 2. Print the three most ordered products.
select sum(order_quantity) as no_of_orders, prod_id from market_star_schema.market_fact_full group by prod_id order by no_of_orders desc limit 3;
-- 3. Print the three least ordered products.
select sum(order_quantity) as no_of_orders, prod_id from market_star_schema.market_fact_full group by prod_id order by no_of_orders limit 3;
-- 4. Find the sales made by the five most profitable products.
select round(sum(sales)) as total_sales, prod_id from market_star_schema.market_fact_full group by prod_id order by total_sales desc limit 5;
-- 5. Arrange the order ids in the order of their recency.
select ord_id,Order_date from market_star_schema.orders_dimen order by Order_Date desc;
-- 6. Arrange all consumers from Coimbatore in alphabetical order.
select customer_name,city from market_star_schema.cust_dimen where city ="Coimbatore" order by customer_name;
...
select concat(product_category," ",product_sub_category) as Category_Subcategory from market_star_schema.prod_dimen;
-- 3. In which month were the most orders shipped?
select count(Ship_id),month(Ship_Date) from  market_star_schema.shipping_dimen group by month(Ship_Date) order by count(Ship_id) desc ;
-- 4. Which month and year combination saw the most number of critical orders?
select count(Ord_id) as no_of_orders, month(order_date) as month_c, year(order_date) as year_c,Order_Priority 
from market_star_schema.orders_dimen where Order_Priority = "critical"  group by month_c,year_c order by no_of_orders desc;
-- 5. Find the most commonly used mode of shipment in 2011.
select count(ship_id)as ship_counts, year(ship_date)as ship_year, ship_mode from market_star_schema.shipping_dimen where year(ship_date) =2011 group by Ship_Mode order by ship_counts desc;

-- -----------------------------------------------------------------------------------------------------------------
-- Regular Expressions

-- 1. Find the names of all customers having the substring 'car'.
select customer_name from market_star_schema.cust_dimen where customer_name regexp  'car';
-- 2. Print customer names starting with A, B, C or D and ending with 'er'.
select customer_name from market_star_schema.cust_dimen where customer_name regexp '^[abcd].*er$';

-- -----------------------------------------------------------------------------------------------------------------
-- Nested Queries

-- 1. Print the order number of the most valuable order by sales.
select ord_id as order_no, round(sales) as rounded_sales 
from market_star_schema.market_fact_full 
where sales = (select max(sales) from market_star_schema.market_fact_full);
-- 2. Return the product categories and subcategories of all the products which don’t have details about the product
-- base margin.
select product_category,product_sub_category from market_star_schema.prod_dimen 
where Prod_id in (select prod_id from market_star_schema.market_fact_full where Product_Base_Margin is null);
-- 3. Print the name of the most frequent customer.
select customer_name, cust_id  from market_star_schema.cust_dimen 
where cust_id = (select cust_id from market_star_schema.market_fact_full group by cust_id  order by count(Cust_id) desc limit 1);
-- 4. Print the three most common products.
select product_category from market_star_schema.prod_dimen
where Prod_id in (select Prod_id from market_star_schema.market_fact_full group by prod_id order by count(prod_id) desc) limit 3;
-- -----------------------------------------------------------------------------------------------------------------
-- CTEs
